...
OCL Compiler Prototype 0.12 adds exact-width bitwise and guarded shift operations.
...
- A platform linker supported by Clang. The LLVM Windows installer includes
  `lld-link`, which is sufficient for Prototype 0.12.
...
ARM64 validation are roadmap work, not 0.12 claims.
...
Prototype 0.12 adds exact-width bitwise and guarded shift operations to the 0.11 language. See [the language specification](docs/OCL_LANGUAGE_SPEC.md) and [architecture overview](docs/ARCHITECTURE.md).
...
0.12's functions, locals, constants, guarded arithmetic and shifts, loop control, bounded local arrays, local structures, and enums, and the
...
target. Windows x86-64 is verified; Windows ARM64 is not yet a 0.12 claim.

...
def _require_known_type(type_name: str, source: str, location: SourceLocation, *, arrays: bool = False, structures=None, enumerations=None) -> None:
    ...
    if arrays and array and array[1] > 256:
        raise DiagnosticError("E0219", "Prototype 0.14 arrays may contain at most 256 elements", source, location)
    ...
    raise DiagnosticError("E0200", f"unknown type '{type_name}'; OCL 0.14 supports scalar, enumeration, and approved local aggregate types", source, location)
...
def _analyze(program: Program, source: str) -> None:
    ...
    if len(program.constants) > 256:
        raise DiagnosticError("E0238", "Prototype 0.14 allows at most 256 top-level constants", source, program.constants[256].location)
    ...
    for function in program.functions:
        ...
        if array_bytes > MAX_LOCAL_ARRAY_BYTES:
            raise DiagnosticError(
                "E0219",
                f"function '{function.name}' declares {array_bytes} bytes of aggregates; Prototype 0.14 allows at most {MAX_LOCAL_ARRAY_BYTES}",
                source,
                function.location,
            )
    ...
    for statement in statements:
        ...
        if isinstance(statement, (LetStatement, VarStatement)):
            ...
            if statement.name in declared:
                raise DiagnosticError("E0210", f"name '{statement.name}' is already declared in this function; OCL 0.14 has no shadowing", source, statement.location)

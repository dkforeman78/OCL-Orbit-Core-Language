...
Prototype 0.12 bitwise operations lower directly to LLVM `and`, `or`, and `xor`; unary complement is `xor` with an all-ones value at the selected width. Shifts branch through an unsigned count check before emitting LLVM `shl`, `ashr`, or `lshr`, so negative and over-width counts trap before LLVM poison can arise. The safe block becomes the recorded value predecessor, preserving valid `phi` nodes when a shift appears inside another control-flow expression. These operations do not alter storage layout or the external ABI.
